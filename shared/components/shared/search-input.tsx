...
export const SearchInput: React.FC<React.PropsWithChildren<Props>> = ({ className }) => {
  ...
  return (
    <>
      {focused && <div className="fixed top-0 left-0 bottom-0 right-0 bg-black/50 z-40" />}

      <div className={cn('flex rounded-2xl flex-1 justify-between relative h-11 z-40', className)} ref={ref}>
        <Search className="absolute top-1/2 translate-y-[-50%] left-3 h-5 text-gray-400" />
        <input
          type="text"
          placeholder="Search..."
          className="w-full rounded-2xl bg-gray-100 outline-none pl-11"
